...
export default function FlowOS() {
  ...
  // ── Boot: handle OAuth callback or check existing token ───────────────────
  useEffect(() => {
  ...
  async function initApp() {
    ...
    if (profile) { setProfile(profile); setScreen("home"); pollTrack(); }
    else { localStorage.clear(); setScreen("connect"); }
  }
  ...
  const submitRating = () => {
    const newSession = {
      id: Date.now(), date: "Just now", track: currentTrack,
      duration: fmt(elapsed), rating: checkinRating,
      ratingLabel: RATING_CONFIG[checkinRating]?.label,
    };
  ...
  if (screen === "loading") return (
    <div style={{ minHeight: "100vh", background: C.bg, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: fontSans }}>
      ...
      <div style={{ maxWidth: 420, margin: "0 auto", paddingBottom: 48 }}>
        ...
        <div style={{ padding: "32px 24px 0", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          ...
          <button onClick={logout}
            style={{ fontSize: 11, color: C.textSoft, background: "none", border: `1px solid ${C.border}`, borderRadius: 8, padding: "6px 10px", cursor: "pointer", fontFamily: fontSans }}>
            Log out
          </button>
        </div>
        ...
        <div style={{ margin: "22px 24px 0", padding: "22px", borderRadius: 18, background: "#fff", border: `1px solid ${C.border}`, boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
          ...
          <div style={{ display: "flex", gap: 14, alignItems: "center", marginBottom: 20 }}>
            {currentTrack.cover
              ? <img src={currentTrack.cover} style={{ width: 50, height: 50, borderRadius: 10, objectFit: "cover" }} />
        ...
        {activeTab === "sessions" && (
          <div style={{ padding: "20px 24px 0" }}>
            ...
            {sessions.length === 0 ? (
              <div style={{ textAlign: "center", padding: "40px 0", color: C.textSoft, fontSize: 14 }}>
                ...
                {sessions.map(s => {
                  ...
                  return (
                    <div key={s.id} style={{ background: "#fff", border: `1px solid ${C.borderSoft}`, borderRadius: 13, padding: "13px 15px", display: "flex", alignItems: "center", gap: 12, boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
                      {s.track.cover
                        ? <img src={s.track.cover} style={{ width: 36, height: 36, borderRadius: 8, objectFit: "cover", flexShrink: 0 }} />
